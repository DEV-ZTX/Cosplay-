    ...
    if chat_id in last_user and last_user[chat_id]['user_id'] == user_id:
        ...
        if last_user[chat_id]['count'] >= 10:
            ...
            if user_id in warned_users and time.time() - warned_users[user_id] < 600:
                ...
                await update.message.reply_text("<blockquote>⚠️ Don't Spam {}...\nYour Messages Will be ignored for 10 Minutes...</blockquote>".format(update.effective_user.first_name), parse_mode='HTML')
...
async def send_image(update: Update, context: CallbackContext) -> None: chat_id = update.effective_chat.id
...
await context.bot.send_photo(
    ...
    caption="""<blockquote>A New {} Cosplay Character Appeared...\n/guess Character Name and add in Your Collection</blockquote>""".format(character['rarity']),
    parse_mode='HTML')
...
if chat_id in first_correct_guesses:
    await update.message.reply_text('<blockquote>❌️ Already Guessed By Someone.. Try Next Time Bruhh</blockquote>', parse_mode='HTML')
    return
...
if sorted(name_parts) == sorted(guess.split()) or any(part == guess for part in name_parts):
    ...
    await update.message.reply_text(f'<blockquote>✅️ Correct Guess! {escape(update.effective_user.first_name)} guessed {last_characters[chat_id]["name"]} from {last_characters[chat_id]["anime"]}</blockquote>', parse_mode='HTML')
